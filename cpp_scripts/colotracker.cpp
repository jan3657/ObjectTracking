...
ColorTracker::ColorTracker(cv::Mat & img, cv::Rect mTrack_window){
    int x1 = mTrack_window.x;
    int y1 = mTrack_window.y;
    int x2 = mTrack_window.x + mTrack_window.width;
    int y2 = mTrack_window.y + mTrack_window.height;
...
void ColorTracker::init(cv::Mat & img, cv::Rect selected)
{
    int x1 = selected.x;
    int y1 = selected.y;
...
cv::Point ColorTracker::histMeanShiftIsotropicScale(double x1, double y1, double x2, double y2, double * scale, int * iter)
{
    ...
    total_fps += double(1) / ((double(clock()) - double(start)) / double(CLOCKS_PER_SEC));
    iterations += 1;
    std::cout<< total_fps/iterations << std::endl; //(clock()-start) << std::endl;
